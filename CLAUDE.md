...
- **`components/AccountListScreen.jsx`**: 메신저 스타일 계좌 목록 화면. 각 계좌가 대화방으로 표시.
- **`components/AccountRoom.jsx`**: 개별 계좌 대화방. 입출금 내역이 말풍선으로 표시되고 AI 채팅 입력 포함.
- **`components/AnimatedBackground.jsx`**: 배경 애니메이션 컴포넌트.
- **`components/DrawerBanner.jsx`**: 드로어 배너 컴포넌트.
- **`components/useWebSocket.js`**: sessionId 기반 WebSocket 연결. 끊기면 3초 후 재연결. (hooks/가 아닌 components/ 하위에 위치)
- **`components/useVoiceInput.js`**: MediaRecorder → `/api/whisper`로 Whisper STT.
- **`components/useVoiceConfirm.js`**: 음성 이체 확인 "네"/"아니오" 감지.
